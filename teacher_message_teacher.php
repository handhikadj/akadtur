                ...
                <div class="row-fluid">
                    ...
                    <div id="block_bg" class="block">
                        ...
                        <div class="block-content collapse in">
							...
							<?php
							$query_announcement = mysqli_query($db, "select * from message
															   LEFT JOIN teacher ON teacher.teacher_id = message.sender_id
															   where message.reciever_id = '$session_id'
															   order by date_sended DESC")or die(mysqli_error($db));
							...
							if ($count_my_message != '0'){
								...
								<?php }}else{ ?>		
								<div class="alert alert-info"><i class="icon-info-sign"></i> Tidak Ada Pesan Baru</div>
								<?php } ?>	
                            </div>
                        </div>
                    </div>
...
<script type="text/javascript">
	$(document).ready( function() {
		...
		var id = $(this).attr("id");
			...
			$('#'+id).modal('hide');
			$.jGrowl("Pesan Terkirim Berhasil Dihapus", { header: 'Hapus Pesan Terkirim' });	
			}
			}); 		
			return false;
		});				
