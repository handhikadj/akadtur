<form id="signin_student" class="form-signin" method="post">
	<h4 class="form-signin-heading"><i class="icon-plus-sign"></i> Tambah Acara</h4>
		...
		<input type="text" class="input-block-level" id="username" name="title" placeholder="Nama Acara" required/>
...
if (isset($_POST['add'])){
	...
	$query = mysqli_query($db, "insert into event (date_end,date_start,event_title,teacher_class_id) values('$date_end','$date_start','$title','$get_id')")or die(mysqli_error($db));
	?>
	<script>
		window.location = "class_calendar.php<?php echo '?id='.$session_id; ?>";
	</script>
<?php
}
?>
...
<table cellpadding="0" cellspacing="0" border="0" class="table" id="">
	...
	<tbody>
	<?php
		$event_query = mysqli_query($db, "select * from event where teacher_class_id = '$get_id' ")or die(mysqli_error($db));
