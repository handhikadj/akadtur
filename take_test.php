...
<?php $query1 = mysqli_query($db, "select * from student_class_quiz where student_id = '$session_id' and class_quiz_id = '$class_quiz_id' ")or die(mysqli_error($db));
...
if ($count > 0){
 ...
 mysqli_query($db, "insert into student_class_quiz (class_quiz_id,student_id,student_quiz_time) values('$class_quiz_id','$session_id','$quiz_time')");
}
 ?>
                     ...
                     <div class="row-fluid">
					    <!-- breadcrumb -->
					     ...
					     <ul class="breadcrumb">
							...
							<li><a href="#"><b>Latihan Quiz</b></a></li>
						</ul>
						 <!-- end breadcrumb -->
                        ...
                        <div id="block_bg" class="block">
							...
							<?php
if($_GET['test'] == 'ok'){
/* $sqlp = mysqli_query($db, "SELECT * FROM groupcode WHERE course_code = '".$row['course_code']."'"); */
$sqlp = mysqli_query($db, "SELECT * FROM class_quiz WHERE class_quiz_id = '$class_quiz_id'")or die(mysqli_error($db));
$rowp = mysqli_fetch_array($sqlp);
/* mysqli_query($db, "UPDATE students SET `time-left` = ".$rowp['time']." WHERE stud_id = '".$_SESSION['user_id']."'"); */
/* echo $rowp['time']; */
$x=0;
?>
										<script>
...
<?php
										$sqla = mysqli_query($db, "select * FROM class_quiz 
										LEFT JOIN quiz ON quiz.quiz_id  = class_quiz.quiz_id
										where teacher_class_id = '$get_id' 
										order by date_added DESC ")or die(mysqli_error($db));
										/* $row = mysqli_fetch_array($sqla); */
										$rowa = mysqli_fetch_array($sqla);
					
										/* $rowa   = $row['quiz_id']; */
/* $sqla = mysqli_query($db, "SELECT * FROM class_quiz WHERE course_code = '".$row['course_code']."'"); */

?>
										<h3>Test Title: <b><?php echo $rowa['quiz_title'];?></b></h3>
										<p><b>Description:<?php echo $rowa['quiz_description'];?></b></p>
										<p></p>
										Waktu Yang Tersisa:<div id="timer">1</div>
										<div id="msg"></div>
					<script>
					jQuery(document).ready(function(){	
...
<?php
	$sqlw = mysqli_query($db, "SELECT * FROM quiz_question where quiz_id = '$quiz_id'  ORDER BY RAND()");
...
<?php
	...
	$sqly = mysqli_query($db, "SELECT * FROM answer WHERE quiz_question_id = '".$roww['quiz_question_id']."'");
...
<?php
	...
	for($x=1;$x<=$x1;$x++){
		...
		$sql = mysqli_query($db, "SELECT * FROM quiz_question WHERE quiz_question_id = ".$x2."");
	...
	<center>
	<h3><br>Skor Kamu Adalah <b><?php echo $score; ?></b> out of <b><?php echo ($x-1); ?></b><br/></h3>
	</center>
	<?php
	/* echo "Your Percentage Grade is : <b>".$per."%</b>"; */
	mysqli_query($db, "UPDATE student_class_quiz SET `student_quiz_time` = 3600, `grade` = '".$score." out of ".($x-1)."' WHERE student_id = '$session_id' and class_quiz_id = '$class_quiz_id'")or die(mysqli_error($db));
?>
<script>
...
<br />
<?php
/* $sql = mysqli_query($db, "SELECT * FROM students WHERE stud_id = '".$_SESSION['user_id']."'");
$row = mysqli_fetch_array($sql);
...
/* 	} else if(!is_null($row['grade'])){
		$sqlg = mysqli_query($db, "SELECT * FROM groupcode WHERE course_code = '".$row['course_code']."'");
		$rowg = mysqli_fetch_array($sqlg);
		echo "You have already taken <b>".$rowg['course_title']."</b> - <b>".$rowg['course_code']."</b> test.";
	}
	if($row['grade']!=''){
		mysqli_query($db, "UPDATE students SET `time-left` = 3600 WHERE stud_id = '".$_SESSION['user_id']."'");
